        ...
        for rid in higher_ids:
            ...
            try:
                with grpc.insecure_channel(addr) as channel:
                    ...
                    if resp.alive:
                        logger.debug(f"[Replica {self.my_id}] Replica {rid} responded OK — deferring.")
                        got_ok = True
                        break
            except Exception:
                logger.debug(f"[Replica {self.my_id}] Replica {rid} unreachable.")
...
def execute_order_2pc(node: BullyNode, order, db_stub, pay_stub):
    ...
    # ------------------------------------------------------------------
    logger.debug(f"{replica_tag} Phase 1 — sending Prepare to DB and Payment...")
    ...
    def prepare_payment():
        ...
        try:
            ...
            logger.debug(f"{replica_tag} Payment Prepare error: {e}")
    ...
    if db_vote and pay_vote:
        logger.debug(f"{replica_tag} Phase 2 — both voted COMMIT. Sending Commit...")

        def commit_db():
            try:
                db_stub.Commit(books_db_pb2.CommitRequest(transaction_id=txn_id), timeout=5.0)
                logger.debug(f"{replica_tag} DB Commit acknowledged.")
            except Exception as e:
                logger.debug(f"{replica_tag} DB Commit error: {e}")

        def commit_payment():
            try:
                pay_stub.Commit(payment_pb2.TransactionId(id=txn_id), timeout=5.0)
                logger.debug(f"{replica_tag} Payment Commit acknowledged.")
            except Exception as e:
                logger.debug(f"{replica_tag} Payment Commit error: {e}")
        ...
        logger.debug(f"{replica_tag} Phase 2 — vote ABORT. Sending Abort to all participants...")

        def abort_db():
            try:
                db_stub.Abort(books_db_pb2.AbortRequest(transaction_id=txn_id), timeout=5.0)
                logger.debug(f"{replica_tag} DB Abort acknowledged.")
            except Exception as e:
                logger.debug(f"{replica_tag} DB Abort error: {e}")

        def abort_payment():
            try:
                pay_stub.Abort(payment_pb2.TransactionId(id=txn_id), timeout=5.0)
                logger.debug(f"{replica_tag} Payment Abort acknowledged.")
            except Exception as e:
                logger.debug(f"{replica_tag} Payment Abort error: {e}")
...
def heartbeat_loop(node: BullyNode):
    while True:
        ...
        try:
            ...
            logger.debug(f"[Replica {node.my_id}] Leader {leader} is DOWN — starting new election.")
            node.leader_id = None
            threading.Thread(target=node.start_election, daemon=True).start()
...
def serve():
    ...
    logger.debug(f"[Replica {my_id}] Starting up. Known replicas: {replicas}")
    ...
    grpc_server.start()
    logger.debug(f"[Replica {my_id}] Inter-replica gRPC server listening on :50056")
    ...
    logger.debug(f"[Replica {my_id}] Waiting {STARTUP_DELAY}s for peers to start...")
